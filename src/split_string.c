 ...
 Freeing the split string requires freeing each pointer within the pointer array and the array itself. */
{
    ...
    char** split_string_array = malloc( sizeof(char *) * string_length);
    ...
    if ( !realloc(split_string_array, j * sizeof(char *)) )
    {
        perror("Error: Failed to realloc split string!");
	struct split_string return_split_string = { .str_p_array = NULL, .num_elements = 0};
